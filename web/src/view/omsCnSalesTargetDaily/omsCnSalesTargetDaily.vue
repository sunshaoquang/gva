        ...
        >
          <template #default="scope">
            ...
            <el-button
              ...
              icon="edit"
              class="table-button"
              @click="updateOmsCnSalesTargetDailyFunc(scope.row)"
              >变更</el-button
            >
            <el-button
              type="primary"
    ...
    >
      <el-scrollbar height="550px">
        <el-descriptions column="1" border>
          ...
          <el-descriptions-item label="销售目标">
            {{ formData.amountTarget }}
          </el-descriptions-item>
...
import {
  ...
  deleteOmsCnSalesTargetDaily,
  deleteOmsCnSalesTargetDailyAll,
  deleteOmsCnSalesTargetDailyByIds,
  updateOmsCnSalesTargetDaily,
  findOmsCnSalesTargetDaily,
  getOmsCnSalesTargetDailyList,
...
const getTableData = async () => {
  const table = await getOmsCnSalesTargetDailyList({
    page: page.value,
...
const deleteRow = (row) => {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    deleteOmsCnSalesTargetDailyFunc(row);
  });
};
...
const onDelete = async () => {
  ...
  const res = await deleteOmsCnSalesTargetDailyByIds({ ids });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
...
const onAllDelete = async () => {
  const res = await deleteOmsCnSalesTargetDailyAll();
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
...
// 更新行
const updateOmsCnSalesTargetDailyFunc = async (row) => {
  const res = await findOmsCnSalesTargetDaily({ ID: row.ID });
  type.value = "update";
  if (res.code === 0) {
...
// 删除行
const deleteOmsCnSalesTargetDailyFunc = async (row) => {
  const res = await deleteOmsCnSalesTargetDaily({ ID: row.ID });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
