        ...
        >
          <template #default="scope">
            ...
            <el-button
              ...
              icon="edit"
              class="table-button"
              @click="updateWmsLogisticsPcs2023MiFunc(scope.row)"
              >变更</el-button
            >
            <el-button
              type="primary"
...
import {
  ...
  deleteWmsLogisticsPcs2023Mi,
  deleteWmsLogisticsPcs2023MiAll,
  deleteWmsLogisticsPcs2023MiByIds,
  updateWmsLogisticsPcs2023Mi,
  findWmsLogisticsPcs2023Mi,
  getWmsLogisticsPcs2023MiList,
...
const getTableData = async () => {
  const table = await getWmsLogisticsPcs2023MiList({
    page: page.value,
...
const deleteRow = (row) => {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    deleteWmsLogisticsPcs2023MiFunc(row);
  });
};
...
const onDelete = async () => {
  ...
  const res = await deleteWmsLogisticsPcs2023MiByIds({ ids });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
...
const onAllDelete = async () => {
  const res = await deleteWmsLogisticsPcs2023MiAll();
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
...
// 更新行
const updateWmsLogisticsPcs2023MiFunc = async (row) => {
  const res = await findWmsLogisticsPcs2023Mi({ ID: row.ID });
  type.value = "update";
  if (res.code === 0) {
...
// 删除行
const deleteWmsLogisticsPcs2023MiFunc = async (row) => {
  const res = await deleteWmsLogisticsPcs2023Mi({ ID: row.ID });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
