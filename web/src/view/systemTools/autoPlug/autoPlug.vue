      ...
      <el-form label-width="140px" class="w-[680px]">
        ...
        <el-form-item>
          ...
          <el-icon
            ...
            @click="
              toDoc(
                'https://www.bilibili.com/video/BV1kv4y1g7nT?p=13&vd_source=f2640257c21e3b547a790461ed94875e'
              )
            "
            ><VideoCameraFilled
          /></el-icon>
...
import { Plus, Minus, VideoCameraFilled } from "@element-plus/icons-vue";
...
const createPlug = async () => {
  ...
  if (res.code === 0) {
    ElMessageBox(
      "创建成功，插件已自动写入后端plugin目录下，请按照自己的逻辑进行创造"
    );
  }
};
