...
func (e *ExcelApi) ImportExcel(c *gin.Context) {
	...
	_ = c.SaveUploadedFile(header, global.GVA_CONFIG.Excel.Dir+fmt.Sprintf("%s__ExcelImport.xlsx",fileName))
...
func (e *ExcelApi) OutputExcel(c *gin.Context) {
	fileName := c.Query("fileName")
	sheetName := c.Query("sheetName")
	createdName := c.Query("createdName")
	// 获取加载的数据
	infos, err := excelService.ParseExcel2InfoList(c,fileName,sheetName,createdName)
	if err != nil {
		global.GVA_LOG.Error("加载数据失败!", zap.Error(err))
		response.FailWithMessage(fmt.Sprintf("加载数据失败_%s",err), c)
		return
	}
	
	//调用上传
	err = excelService.OutputExcelInfoListData(infos,fileName)
	if err != nil {
		global.GVA_LOG.Error("上传数据失败!", zap.Error(err))
...
func (e *ExcelApi) LoadExcel(c *gin.Context) {
	fileName := c.Query("fileName")
	menus, err := excelService.ParseExcel2InfoList(c,fileName,"","")
	if err != nil {
		global.GVA_LOG.Error("加载数据失败!", zap.Error(err))
